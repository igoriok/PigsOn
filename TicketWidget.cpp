    ...
    m_ui(new Ui::TicketWidget)
{
    ...
    m_ui->comboBox_Status->addItem("Open", QString("open"));
...
void TicketWidget::updateContent(const Ticket & ticket)
{
    ...
    m_ui->comboBox_Priority->setCurrentIndex(ticket.Priority - 1);
...
void TicketWidget::on_pushButton_Update_clicked()
{
    ...
    else if (QMessageBox::information(this, tr("Alert"), tr("Proceed?"), QMessageBox::Ok, QMessageBox::Cancel) == QMessageBox::Ok)
    {
        ...
        ticket.AssignedTech.insert(QString(), m_ui->comboBox_Person->itemData(m_ui->comboBox_Person->currentIndex(), Qt::UserRole).toString());
...
void TicketWidget::on_comboBox_Group_currentIndexChanged(int index)
{
    ...
    if (index != -1)
    {
        ...
        if (id ==  ticket.AssignedGroup)
        {
            ...
            box->setCurrentIndex(box->findData(QVariant(ticket.AssignedTech.value(QString())), Qt::UserRole));
        } else
        {
